...
public class SystemController {
    ...
    @Value("${jmx.username}")
    private String JMX_USER;
    @Value("${jmx.password}")
    private String JMX_PASSWORD;
    @Value("${james.default_domain}")
    ...
    public String loginDo(@RequestParam Integer menu) {
        ...
        switch (menu) {
            case CommandType.LOGIN:
                ...
                // 가상 호스팅 지원: 입력한 ID에 도메인이 없으면 기본 도메인 결합
                if (userid != null && !userid.contains("@") && DEFAULT_DOMAIN != null && !DEFAULT_DOMAIN.isEmpty()) {
                    userid = userid + "@" + DEFAULT_DOMAIN;
                }

                // Check the login information is valid using <<model>>ImapAgent.
                ImapAgent imapAgent = new ImapAgent(host, userid, password);
                boolean isLoginSuccess = imapAgent.validate();
    ...
    protected boolean isAdmin(String userid) {
        ...
        if (userid != null) {
            if (userid.equals(this.ADMINISTRATOR) || userid.startsWith(this.ADMINISTRATOR + "@")) {
                status = true;
            }
        }
    ...
    public String mainMenu(Model model) {
        ImapAgent imap = new ImapAgent();
        imap.setHost((String) session.getAttribute("host"));
        imap.setUserid((String) session.getAttribute("userid"));
        imap.setPassword((String) session.getAttribute("password"));

        String messageList = imap.getMessageList();
    ...
    public String adminMenu(Model model) {
        log.debug("root.id = {}, root.password = {}, admin.id = {}, jmx.user = {}",
                ROOT_ID, ROOT_PASSWORD, ADMINISTRATOR, JMX_USER);
    ...
    public String addUserDo(@RequestParam String id, @RequestParam String password,
        ...
        try {
            ...
            UserAdminAgent agent = new UserAdminAgent("127.0.0.1", JAMES_CONTROL_PORT, cwd,
                    JMX_USER, JMX_PASSWORD, ADMINISTRATOR, DEFAULT_DOMAIN);
