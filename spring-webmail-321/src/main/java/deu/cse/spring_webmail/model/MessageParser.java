    ...
    private void getEnvelope(Message m) throws Exception {
        ...
        if (froms != null && froms.length > 0) {
            ...
            if (from instanceof InternetAddress) {
                ...
                String raw = MimeUtility.decodeText(from.toString());
                try {
                    InternetAddress parsed = new InternetAddress(raw);
                    String addr = parsed.getAddress();
                    fromAddress = (addr != null) ? addr : raw;
                } catch (Exception e) {
                    fromAddress = raw;
                }
            }
    ...
    private String getAddresses(Address[] addresses) {
        ...
        for (Address address : addresses) {
            if (address instanceof InternetAddress) {
                buffer.append(((InternetAddress) address).toUnicodeString());
            } else {
